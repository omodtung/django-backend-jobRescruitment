...
class ResumeSerializers(serializers.ModelSerializer):
    ...
    class Meta:
        model = Resume  
        fields = [
            "_id", "email", "userId", "user", "company", "job", "history", "url", "status",
            "startDate", "endDate",
    ...
    def create(self, validated_data):
        ...
        data["company"] = CompaniesSerializer(new_resume.company).data if new_resume.company else None
        data["job"] = JobSerializers(new_resume.job).data if new_resume.job else None
        return {
                "code": 0,
